...
const Profile = () => {
  ...
  useEffect(() => {
    const fetchTutor = async () => {
      try {
        ...
        const response = await api.get(`/guru/by-user/${user.id}`);
        setTutor(response.data?.data || null);
      } catch (err) {
  ...
  if (!tutor) {
    return (
      <div className={styles.container}>
        <div className={styles.errorState}>Data tutor tidak ditemukan</div>
      </div>
    );
  }
    ...
    <div className={styles.container}>
      ...
      <div className={styles.header}>
        ...
        <div className={styles.headerInfo}>
          <h1 className={styles.headerName}>{tutor.nama_tutor}</h1>
          <p className={styles.headerRole}>Tutor</p>
          {tutor.nip && <p className={styles.headerNip}>NIP: {tutor.nip}</p>}
        </div>
      </div>
      ...
      <div className={styles.card}>
        ...
        <div className={styles.infoGrid}>
          <div className={styles.infoItem}>
            ...
            <span className={styles.infoValue}>{tutor.nama_tutor || '—'}</span>
          </div>
          <div className={styles.infoItem}>
            <span className={styles.infoLabel}>Username</span>
            <span className={styles.infoValue}>{tutor.username || '—'}</span>
          </div>
          <div className={styles.infoItem}>
            <span className={styles.infoLabel}>Tempat Lahir</span>
            <span className={styles.infoValue}>{tutor.tempat_lahir || '—'}</span>
          </div>
          <div className={styles.infoItem}>
            <span className={styles.infoLabel}>Tanggal Lahir</span>
            <span className={styles.infoValue}>{formatTanggal(tutor.tanggal_lahir)}</span>
          </div>
          <div className={styles.infoItem}>
            <span className={styles.infoLabel}>Jenis Kelamin</span>
            <span className={styles.infoValue}>{JENIS_KELAMIN_MAP[tutor.jenis_kelamin] || '—'}</span>
          </div>
          <div className={styles.infoItem}>
            <span className={styles.infoLabel}>Pendidikan</span>
            <span className={styles.infoValue}>{tutor.pendidikan || '—'}</span>
          </div>
          <div className={styles.infoItem}>
            <span className={styles.infoLabel}>Nomor Telepon</span>
            <span className={styles.infoValue}>{tutor.no_hp || '—'}</span>
          </div>
          <div className={styles.infoItem}>
          ...
          <div className={styles.infoItem}>
            ...
            <span className={`${styles.infoValue} ${tutor.status === 'Aktif' ? styles.statusAktif : styles.statusNonaktif}`}>
              {tutor.status || '—'}
            </span>
          </div>
          <div className={styles.infoItem}>
            <span className={styles.infoLabel}>Tanggal Bergabung</span>
            <span className={styles.infoValue}>{formatTanggal(tutor.tanggal_bergabung)}</span>
          </div>
          <div className={styles.infoItemFull}>
            <span className={styles.infoLabel}>Alamat</span>
            <span className={styles.infoValue}>{tutor.alamat || '—'}</span>
          </div>
          <div className={styles.infoItemFull}>
            <span className={styles.infoLabel}>Jadwal</span>
            <span className={styles.infoValue}>{tutor.jadwal || '—'}</span>
          </div>
        </div>
      </div>
